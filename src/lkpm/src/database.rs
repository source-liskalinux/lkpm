...
impl Database {
    pub fn load(cfg: &Config) -> Result<Self, LkpmError> {
        let db_file = cfg.db_path.join("db.sqlite");
        if !db_file.exists() {
            fs::create_dir_all(&cfg.db_path).map_err(LkpmError::Io)?;
        }
        let conn = Connection::open(&db_file)
            .map_err(|e| LkpmError::Other(format!("db.sqlite open error: {}", e)))?;
        conn.execute_batch(
        ...
        .map_err(|e| LkpmError::Other(format!("db.sqlite init error: {}", e)))?;
        conn.execute_batch(
    ...
    fn conn(&self) -> Result<Connection, LkpmError> {
        Connection::open(&self.db_file).map_err(|e| LkpmError::Other(format!("db.sqlite open error: {}", e)))
    }
    pub fn register(&mut self, _cfg: &Config, package: InstalledPackage) -> Result<(), LkpmError> {
        ...
        let files_json = serde_json::to_string(&package.files)
            .map_err(|e| LkpmError::Other(format!("db.sqlite serialize files error: {}", e)))?;
        let requires_json = serde_json::to_string(&package.requires)
            .map_err(|e| LkpmError::Other(format!("db.sqlite serialize requires error: {}", e)))?;
        let optdepends_json = serde_json::to_string(&package.optdepends)
            .map_err(|e| LkpmError::Other(format!("db.sqlite serialize optdepends error: {}", e)))?;
        let conflicts_json = serde_json::to_string(&package.conflicts)
            .map_err(|e| LkpmError::Other(format!("db.sqlite serialize conflicts error: {}", e)))?;
        let provides_json = serde_json::to_string(&package.provides)
            .map_err(|e| LkpmError::Other(format!("db.sqlite serialize provides error: {}", e)))?;
        conn.execute(
        ...
        .map_err(|e| LkpmError::Other(format!("db.sqlite insert error: {}", e)))?;
        Ok(())
    }
    pub fn remove(&mut self, _cfg: &Config, name: &str) -> Result<Option<InstalledPackage>, LkpmError> {
        if let Some(pkg) = self.get(name)? {
            ...
            conn.execute("DELETE FROM installed_packages WHERE name = ?1", params![name])
                .map_err(|e| LkpmError::Other(format!("db.sqlite delete error: {}", e)))?;
            Ok(Some(pkg))
        } else {
            Ok(None)
        }
    ...
    pub fn get(&self, name: &str) -> Result<Option<InstalledPackage>, LkpmError> {
        ...
        let mut stmt = conn
            ...
            .map_err(|e| LkpmError::Other(format!("db.sqlite query prepare error: {}", e)))?;
        let mut rows = stmt
            ...
            .map_err(|e| LkpmError::Other(format!("db.sqlite query error: {}", e)))?;
        if let Some(res) = rows.next() {
            let pkg = res.map_err(|e| LkpmError::Other(format!("db.sqlite row error: {}", e)))?;
            Ok(Some(pkg))
        } else {
            Ok(None)
        }
    ...
    pub fn list(&self) -> Result<Vec<InstalledPackage>, LkpmError> {
        ...
        for r in rows {
            results.push(r.map_err(|e| LkpmError::Other(format!("db.sqlite row error: {}", e)))?);
        }
        Ok(results)
    }
    pub fn store_repo_index(cfg: &Config, index_url: &str, index_json: &str) -> Result<(), LkpmError> {
        let db_file = cfg.db_path.join("db.sqlite");
        let conn = Connection::open(&db_file).map_err(|e| LkpmError::Other(format!("db.sqlite open error: {}", e)))?;
        conn.execute(
    ...
    pub fn read_repo_index(cfg: &Config, index_url: &str) -> Result<Option<String>, LkpmError> {
        ...
        if let Some(r) = rows.next() {
            let s: String = r.map_err(|e| LkpmError::Other(format!("db.sqlite row error: {}", e)))?;
            Ok(Some(s))
        } else {
            Ok(None)
        }
